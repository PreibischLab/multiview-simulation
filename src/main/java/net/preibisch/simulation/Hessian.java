...
public class Hessian
{
	...
	public static Pair< Img< FloatType >, Img< FloatType > > largestEigenVector( final RandomAccessibleInterval< FloatType > randomAccessible )
	{
		...
		while ( cursor.hasNext() )
		{
			...
			if ( cursor.getIntPosition( 0 ) == 208 && ( cursor.getIntPosition( 1 ) == 80 || cursor.getIntPosition( 1 ) == 81 ) && cursor.getIntPosition( 2 ) == 33 )
			//if ( cursor.getIntPosition( 0 ) == 128 && cursor.getIntPosition( 1 ) == 37 && cursor.getIntPosition( 2 ) == 165 )
			{
				...
				System.out.println( "biggestEigenValue: " + eigenValue );
				System.out.println( "biggestEigenVector: " + Util.printCoordinates( eigenVector ) + "\n");
			}
			...
			double[] result = evDec.getImagEigenvalues();
			...
			if ( !found )
			{
				...
				{
					System.out.println( cursor.getIntPosition( 0 ) + " " + cursor.getIntPosition( 1 )+ " " + cursor.getIntPosition( 2 ) + " " + cursor.get().get() );
					System.out.println( "biggestEigenValueIndex: " + biggestEigenValueIndex + " = " + biggestEigenValue );
					System.out.println( Util.printCoordinates( ev ) );
					System.out.println( evec.get( 0, biggestEigenValueIndex ) + " " + evec.get( 1, biggestEigenValueIndex ) + " " + evec.get( 2, biggestEigenValueIndex ) );
					System.out.println( Util.printCoordinates( evec.getColumnPackedCopy() ) + "\n");
				}

				raEVec.setPosition( posN );
				raEVec.get().set( (float)evec.get( 0, biggestEigenValueIndex ) );

				raEVec.fwd( n );
				raEVec.get().set( (float)evec.get( 1, biggestEigenValueIndex ) );

				raEVec.fwd( n );
				raEVec.get().set( (float)evec.get( 2, biggestEigenValueIndex ) );

				raEVal.setPosition( cursor );
				raEVal.get().set( (float)biggestEigenValue );
			}
			else
			{
